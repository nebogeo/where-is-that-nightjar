...
(define (rect-button x y w h jitter callback)
      (list "rect-button" x y w h
            jitter callback #f))))

(define (image-button x y jitter callback)
  (let ((image-name (choose (list "button-01" "button-02"))))
    (let ((image (find-image image-name image-lib)))
      (list "image-button" x y 
            image.width 
            image.height
            jitter callback image-name))))

(define (circle-button x y r callback)
  (list "circle-button" x y r r #f callback #f))
...
(define (button-jitter b) (list-ref b 5))
(define (button-callback b) (list-ref b 6))
(define (button-image b) (list-ref b 7))
